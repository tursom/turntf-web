...
登录后从侧栏进入「Raft 数据库」（`/raft`）。管理员在同一行先搜索用户（用户名、昵称、登录名或 ID），再搜索并选择该用户拥有的数据库；普通用户默认查看自己的数据库。选择框以下拉浮层展示候选项，选中后收起。

选库后可按键名前缀查询，新增或编辑键值，支持文本、JSON 和 Base64。版本号使用无损整数解析；保存通过事务条件检查键是否不存在或修改版本是否一致。冲突和网络错误保留草稿，键被并发删除时可保留草稿重新创建或改名保存；切换用户、数据库、键及重新读取前会提示未保存的修改。格式切换按实际字节判断修改状态，后台刷新不会覆盖编辑器内容。

「新建我的数据库」始终使用当前登录用户作为所有者，数据库名称在集群内唯一。「管理授权」支持读取授权列表、设置独立的读/写/管理授权权限以及撤销授权；系统管理员可以管理其他用户的数据库，普通用户仍受数据库 ACL 限制。

后端须启用 KV 共识并运行包含数据库列表、ACL 查询及管理员权限的新版服务。读取的是当前节点已应用的数据；当前后端写入须连接 Raft leader，不会自动转发。接口语义见服务端 `docs/raft-database-web.md`。

浏览器回归使用真实页面、Ant Design 和 HTTP 封装，并模拟 API 数据，覆盖桌面及手机布局：

```bash
npm run dev -- --host 127.0.0.1 --port 5186 --strictPort
# 另一个终端；Playwright 可从独立测试环境提供，不增加生产依赖
...
| `/contacts` | 联系人 | 登录 |
| `/raft` | Raft 数据库 | 登录；管理员可按用户管理，其他操作受数据库 ACL 限制 |
| `/settings` | 个人设置 | 登录 |
| `/admin` | 仪表盘 | 管理员 |
